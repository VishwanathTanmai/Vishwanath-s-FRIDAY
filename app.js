...
recognition.onresult = function (event) {
  ...
  if(localStorage.getItem("lang") === "en-US"){
    ...
    if (transcript.includes("what is current time")) {
      readOut(currentTime);
    }
    if (transcript.includes("what is connection status")) {
    ...
    if (transcript.includes("Tell about yourself")) {
      readOut(
        "sir, i am a Vishwanath's Friday, a voice asistant made for browsers using AI . I can do anything which can be done from a browser."
      );
    }
    ...
    // weather report
    if (
      transcript.includes("what is today temperature")
    ) {
  ...
  // spotify
   ...
   if (transcript.includes("how to")|| transcript.includes('who is') || transcript.includes('what are')) {
    ...
    // news commands
    if (transcript.includes("what is today news")) {
